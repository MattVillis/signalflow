...
//   SF_APP_URL=https://mattvillis.github.io/signalflow/  (--open target
//     when no signalflow.html sits next to the bridge)
//   SF_BIND=0.0.0.0  (127.0.0.1 to keep the bridge off the LAN — no sessions)
//   SF_SHARE_AI=1    (let LAN/tunnel session members use this machine's AI engine)
...
// Sessions need LAN reach, so the bridge binds every interface; the AI engine
// and maintainer publish stay loopback-only (see isLocal) unless SF_SHARE_AI=1.
const BIND = process.env.SF_BIND || "0.0.0.0";
...
const APP_URL = process.env.SF_APP_URL || "https://mattvillis.github.io/signalflow/";
const APP_FILE = join(dirname(SCRIPT), "signalflow.html");
const SYNC_FILE = join(dirname(SCRIPT), "sf-sync.mjs");
function openApp() {
...
/* ---- sessions: LAN addresses + optional internet tunnel (cloudflared) ---- */
const lanIps = () => Object.values(networkInterfaces()).flat().filter(i => i && i.family === "IPv4" && !i.internal && !i.address.startsWith("169.254.")).map(i => i.address);
// a request is local only if it came over loopback AND was not forwarded by the tunnel
const isLocal = req => /^(::1|127\.0\.0\.1|::ffff:127\.0\.0\.1)$/.test(req.socket.remoteAddress || "") && !req.headers["cf-connecting-ip"] && !req.headers["x-forwarded-for"];
let tunnel = { proc: null, url: null, error: null };
function startTunnel() {
  return new Promise(resolve => {
    if (tunnel.url) return resolve({ ok: true, url: tunnel.url });
    let proc;
    try { proc = spawn("cloudflared", ["tunnel", "--url", `http://127.0.0.1:${PORT}`, "--no-autoupdate"], { windowsHide: true, stdio: ["ignore", "pipe", "pipe"] }); }
    catch (e) { return resolve({ ok: false, error: e.message }); }
    tunnel = { proc, url: null, error: null };
    const timer = setTimeout(() => { if (!tunnel.url) { proc.kill(); tunnel = { proc: null, url: null, error: "cloudflared gave no URL in 25 s" }; resolve({ ok: false, error: tunnel.error }); } }, 25000);
    const scan = d => { const m = /https:\/\/[a-z0-9-]+\.trycloudflare\.com/.exec(String(d)); if (m && !tunnel.url) { tunnel.url = m[0]; clearTimeout(timer); console.log(`[sf-bridge] tunnel up: ${tunnel.url}`); resolve({ ok: true, url: tunnel.url }); } };
    proc.stdout.on("data", scan); proc.stderr.on("data", scan);
    proc.on("error", e => { clearTimeout(timer); tunnel = { proc: null, url: null, error: e.code === "ENOENT" ? "cloudflared is not installed — winget install Cloudflare.cloudflared (Windows) / brew install cloudflared (macOS)" : e.message }; resolve({ ok: false, error: tunnel.error }); });
    proc.on("exit", () => { if (tunnel.proc === proc) { console.log("[sf-bridge] tunnel closed"); tunnel = { proc: null, url: null, error: null }; } });
  });
}
function stopTunnel() { if (tunnel.proc) { try { tunnel.proc.kill(); } catch { /* gone */ } } tunnel = { proc: null, url: null, error: null }; }
...
if (sub === "status") { ping().then(up => { console.log(up ? `[sf-bridge] running on :${PORT}` : "[sf-bridge] not running"); process.exit(up ? 0 : 1); }); }
  ...
  const server = http.createServer((req, res) => {
    ...
    if (req.method === "GET" && req.url.startsWith("/status")) {
      ...
      return res.end(JSON.stringify({ ok: true, engine: ENGINE.note, publish: isLocal(req) && canPublish(), ai: isLocal(req) || !!process.env.SF_SHARE_AI,
        sync: !!sync, lan: lanIps(), port: PORT, tunnel: tunnel.url, rooms: sync ? sync.rooms() : [] }));
    }
    // the app itself, so a session link is just http://<host>:PORT/?session=<room>
    if (req.method === "GET" && /^\/(index\.html)?(\?.*)?$/.test(req.url) && existsSync(APP_FILE)) {
      res.writeHead(200, { "Content-Type": "text/html; charset=utf-8", "Cache-Control": "no-store" });
      // tag the page so the app talks to THIS bridge (LAN ip / tunnel), not localhost
      return res.end(readFileSync(APP_FILE, "utf8").replace("<body>", "<body>\n<script>window.SF_BRIDGE=location.origin;window.SF_SERVED=1;</script>"));
    }
    if (req.method === "POST" && req.url.startsWith("/tunnel")) {
      if (!isLocal(req)) { res.writeHead(403, { "Content-Type": "application/json" }); return res.end('{"ok":false,"error":"host only"}'); }
      const stop = req.url.startsWith("/tunnel/stop");
      if (stop) { stopTunnel(); res.writeHead(200, { "Content-Type": "application/json" }); return res.end('{"ok":true}'); }
      return startTunnel().then(r => { res.writeHead(200, { "Content-Type": "application/json" }); res.end(JSON.stringify(r)); });
    }
    ...
    if (!isLocal(req) && !process.env.SF_SHARE_AI) { res.writeHead(403, { "Content-Type": "application/json" }); return res.end(JSON.stringify({ ok: false, error: "the host's AI engine is not shared with session members (host: set SF_SHARE_AI=1 to allow)" })); }
    let body = "";
  ...
  server.listen(PORT, BIND, async () => {
    console.log(`[sf-bridge] listening on http://localhost:${PORT}${BIND === "0.0.0.0" ? " (+ LAN: " + (lanIps().join(", ") || "none") + ")" : ""}`);
    try {
      const mod = await import(pathToFileURL(SYNC_FILE).href);
      sync = mod.attachSync(server, m => console.log("[sf-bridge] " + m));
      console.log(`[sf-bridge] sessions: ready (${sync.saved().length} saved) — share http://<this-ip>:${PORT}/?session=<room>`);
    } catch (e) { console.log(`[sf-bridge] sessions: unavailable (${existsSync(SYNC_FILE) ? e.message : "sf-sync.mjs not found next to the bridge"})`); }
